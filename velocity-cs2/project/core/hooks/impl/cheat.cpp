	...
	void __fastcall cheat::create_move( std::uintptr_t thisptr, int slot, bool active )
	{
		...
		systems::g_input.update( );
		{
...

			{
				...
				features::movement::g_test_strafer.store_angles( );
...

			{
...

				{
					...
					// Autostop must run AFTER rage/legit so it sees the final
					// "should_stop" decision and corrects movement on the fire tick.
					diag::set_exception_phase( "create_move: autostop" );
