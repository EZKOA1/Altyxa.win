...
namespace features::esp::other {
	...
	void overlay::add_bomb (xdraw::draw_list& draw_list) {
		...
		// ==================== œ–Œ«–¿◊Õ€… ‘ŒÕ ====================
		const auto blur_alpha = 150;
		draw_list.rect_filled_blurred (x, y, total_w, h, xdraw::corner_radius {r}, xdraw::color {255, 255, 255, static_cast<std::uint8_t>(blur_alpha)});

		auto bg = s.window_bg;
		bg.a = static_cast<std::uint8_t>(bg.a * 0.5f);
		draw_list.rect_filled (x, y, total_w, h, bg, xdraw::corner_radius {r});
		// ======================================================

		auto cx = x + inner_pad;

		// Site
		auto child_bg = s.child_bg;
		child_bg.a = static_cast<std::uint8_t>(child_bg.a * 0.7f);
		draw_list.rect_filled (cx, y + inner_pad, site_pill_w, inner_h, child_bg, xdraw::corner_radius {inner_r});
		...
		draw_list.rect_filled (cx, y + inner_pad, health_pill_w, inner_h, child_bg, xdraw::corner_radius {inner_r});
		...
		draw_list.rect_filled (cx, y + inner_pad, timer_pill_w, inner_h, child_bg, xdraw::corner_radius {inner_r});
	...
	void overlay::add_spectators (xdraw::draw_list& draw_list) {
		...
		// ==================== œ–Œ«–¿◊Õ€… «¿√ŒÀŒ¬Œ  ====================
		const auto blur_alpha_header = 150;
		draw_list.rect_filled_blurred (x, ry, header_w, header_h, xdraw::corner_radius {r}, xdraw::color {255, 255, 255, static_cast<std::uint8_t>(blur_alpha_header)});
		auto bg_header = s.window_bg;
		bg_header.a = static_cast<std::uint8_t>(bg_header.a * 0.5f);
		draw_list.rect_filled (x, ry, header_w, header_h, bg_header, xdraw::corner_radius {r});

		const auto htx = x + inner_pad;
		const auto htw = header_tw + text_pad_x * 2.0f;
		auto child_bg = s.child_bg;
		child_bg.a = static_cast<std::uint8_t>(child_bg.a * 0.7f);
		draw_list.rect_filled (htx, ry + inner_pad, htw, header_inner_h, child_bg, xdraw::corner_radius {inner_r});
		...
		for (auto i = 0; i < count; ++i) {
			...
			// ==================== œ–Œ«–¿◊Õ¿ﬂ —“–Œ ¿ ====================
			const auto blur_alpha_row = 150;
			draw_list.rect_filled_blurred (rx, ry, row_w, row_h, xdraw::corner_radius {r}, xdraw::color {255, 255, 255, static_cast<std::uint8_t>(blur_alpha_row)});
			auto bg_row = s.window_bg;
			bg_row.a = static_cast<std::uint8_t>(bg_row.a * 0.5f);
			draw_list.rect_filled (rx, ry, row_w, row_h, bg_row, xdraw::corner_radius {r});

			auto cx = rx + inner_pad;

			auto child_bg_row = s.child_bg;
			child_bg_row.a = static_cast<std::uint8_t>(child_bg_row.a * 0.7f);
			draw_list.rect_filled (cx, ry + inner_pad, name_pill_w, inner_h, child_bg_row, xdraw::corner_radius {inner_r});
