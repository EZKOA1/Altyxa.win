...
namespace features::combat {
	...
	void rage::on_create_move( systems::input::usercmd* cmd )
	{
		...
		if ( settings::g_combat.m_ragebot.enabled && !is_knife && !is_taser )
		{
			const auto& rb_cfg = settings::g_combat.m_ragebot.get_group( ctx.weapon_type );
			const auto weapon_needs_scope = ctx.weapon_type == cstypes::weapon_type::sniper ||
				ctx.item_def_idx == cstypes::item_definition_index::weapon_aug ||
				ctx.item_def_idx == cstypes::item_definition_index::weapon_sg_553 ||
				ctx.item_def_idx == cstypes::item_definition_index::weapon_g3sg1 ||
				ctx.item_def_idx == cstypes::item_definition_index::weapon_scar_20;

			if ( rb_cfg.autoscope.value && weapon_needs_scope && !ctx.is_scoped )
			{
				constexpr auto scope_btn = cstypes::command_buttons::in_second_attack;
				cmd->buttons.value |= scope_btn;
				cmd->buttons.value_changed |= scope_btn;
				cmd->buttons.value_scroll |= scope_btn;
			}
		}
		...
		else
		{
			...
			const auto can_fire_now = g_shared.can_shoot( cmd, local.controller );

			this->run_gun( cmd, aim_ctx, local, can_fire_now );
		}
	}
	...
	void rage::run_gun( systems::input::usercmd* cmd, const aim_context& ctx, const systems::local::snapshot& local, bool allow_fire )
	{
		...
		if ( config.no_spread.value )
		{
			...
			const auto accurate = best.hitchance >= needed_hc;
			const auto max_acc = g_shared.is_max_accuracy( shared_ctx.inaccuracy );
			...
			bool allow_move_due_cooldown{ false };
			if ( config.autostop_move_between_shots.value && !shot_viable )
			{
				const auto base_cmd = cmd->csgo_user_cmd.mutable_base( );
				if ( base_cmd )
				{
					...
					const auto next_primary = memory::read<int>( shared_ctx.weapon + SCHEMA( "C_BasePlayerWeapon", "m_nNextPrimaryAttackTick"_hash ) );
					const auto stop_ticks = g_shared.calculate_stop_ticks( prestate.networked_velocity, shared_ctx.weapon_max_speed, local.pawn );
					const auto ticks_until_fire = next_primary - client_tick;
					allow_move_due_cooldown = ticks_until_fire > stop_ticks + 2;
				}
			}

			const auto needs_stop = best.valid && this->should_stop_movement( ctx ) && !allow_move_due_cooldown;
			if ( needs_stop )
			{
				if ( !shot_viable )
				{
			...
			if ( config.autostop_early.value && !this->m_should_stop && !shot_viable && best.valid && this->should_stop_movement( ctx ) && !allow_move_due_cooldown )
			{
				this->m_should_stop = true;
			}

			if ( shot_viable && allow_fire && config.autostop.value )
			{
				const auto velocity = prestate.networked_velocity;
				if ( velocity.length_2d( ) > 0.5f || ( !ctx.on_ground && config.autostop_in_air.value ) )
				{
					this->m_should_stop = true;
				}
			}
		...
		// Move between shots: skip stopping if we cannot fire this tick yet (fire-rate cooldown is active).
		// Re-enable the stop planning when approaching the next fire window.
		bool allow_move_due_cooldown{ false };
		if ( config.autostop_move_between_shots.value && !shot_viable )
		{
			const auto base_cmd = cmd->csgo_user_cmd.mutable_base( );
		...
		if ( needs_stop )
		{
			if ( !shot_viable )
			{
				...
				if ( stop )
				{
					...
					this->m_should_stop = planned.valid || config.autostop_early.value;
				}
				else
				{
					this->m_should_stop = best.valid;
			...
			{
				// Shot is viable this tick: force stop so the autostop code runs
				// and counteracts any residual movement right before firing.
				this->m_should_stop = true;
			}
		}

		// Early mode: if a valid target exists before the shot is viable, start stopping
		// immediately so we arrive at stopped velocity as soon as possible.
		if ( config.autostop_early.value && !this->m_should_stop && !shot_viable && best.valid && this->should_stop_movement( ctx ) && !allow_move_due_cooldown )
		{
			const auto stop = this->predict_stop( ctx, primary_eye, local );
			if ( stop )
			{
				...
				auto planned_hits = scan_from_eye_candidates( future_offset, stop->inaccuracy );
				const auto planned = this->select_best( ctx, planned_hits, stop->inaccuracy );
				const auto planned_hc = planned.valid ? this->evaluate_hitchance( planned.hit, ctx, stop->inaccuracy ) : 0.0f;
				if ( planned.valid || planned_hc >= needed_hc * 0.5f )
				{
					this->m_should_stop = true;
				}
			}
			else if ( config.autostop_in_air.value )
			{
				this->m_should_stop = true;
			}
			else
			{
				// Fallback: we have a target and need to stop early to be on target.
				this->m_should_stop = true;
			}
		}
		...
		// We are actually going to fire this tick: ensure autostop runs so
		// any residual input movement is negated on the fire tick.
		if ( ready_to_fire && allow_fire && config.autostop.value )
		{
			const auto velocity = prestate.networked_velocity;
			if ( velocity.length_2d( ) > 0.5f || ( !ctx.on_ground && config.autostop_in_air.value ) )
			{
	...
	bool rage::should_stop_movement( const aim_context& ctx ) const
	{
		...
		if ( config.autostop_in_air.value )
		{
			const auto speed_2d = velocity.length_2d( );
			return speed_2d > 1.0f;
		}
	...
	void rage::update_penetration_crosshair( const systems::local::snapshot& local )
	{
			...
			|| ctx.weapon_type < cstypes::weapon_type::pistol || ctx.weapon_type > cstypes::weapon_type::lmg )
		{
			...
			this->m_penetration_crosshair_world.valid = false;
			return;
		}
		...
		if ( cfg.physics.value )
		{
			constexpr auto max_trace_range{ 8000.0f };
			const auto end = eye_pos + forward * max_trace_range;
			const auto filter = systems::g_tracing.make_filter( local.pawn, 0x001c1003, 4 );
			const auto trace = systems::g_tracing.trace( eye_pos, end, filter );

			if ( trace.fraction < 1.0f || trace.all_solid )
			{
				this->m_penetration_crosshair_world.position = trace.position;
				this->m_penetration_crosshair_world.valid = true;
			}
			else
			{
				this->m_penetration_crosshair_world.position = end;
				this->m_penetration_crosshair_world.valid = true;
			}
		}
		else
	...
	void rage::draw_penetration_crosshair( xdraw::draw_list& draw_list ) const
	{
		...
		if ( cfg.physics.value && this->m_penetration_crosshair_world.valid )
		{
			const auto screen = systems::g_view.project( this->m_penetration_crosshair_world.position );
			if ( systems::g_view.projection_valid( screen ) )
			{
				cx = std::floorf( screen.x );
				cy = std::floorf( screen.y );

				const auto local_eye = g_shared.get_eye_position( local.pawn );
				const auto dist = ( this->m_penetration_crosshair_world.position - local_eye ).length( );
				scale = std::clamp( 1200.0f / std::max( dist, 1.0f ), 0.3f, 2.5f );
			}
			else
			{
				const auto [ screen_w, screen_h ] = xdraw::viewport_size( );
				cx = std::floorf( static_cast< float >( screen_w ) * 0.5f );
				cy = std::floorf( static_cast< float >( screen_h ) * 0.5f );
			}
		}
		else
		{
		...
		const auto half_size = cfg.size.value * 0.5f * scale;
		const auto outline_size = cfg.outline_thickness.value * scale;
		const auto style = cfg.crosshair_style.value;
		...
		if ( cfg.glow )
		{
			...
			switch ( style )
			{
			case settings::combat::penetration_crosshair::style::square:
				...
				break;
			case settings::combat::penetration_crosshair::style::circle:
			case settings::combat::penetration_crosshair::style::circle_outline:
				glow.circle_filled( cx, cy, half_size + outline_size, glow_col );
				break;
			}
		}
		...
		case settings::combat::penetration_crosshair::style::circle:
			draw_list.circle_filled( cx, cy, half_size + outline_size, outline );
			draw_list.circle_filled( cx, cy, half_size, fill );
			break;

		case settings::combat::penetration_crosshair::style::circle_outline:
			draw_list.circle( cx, cy, half_size, outline, outline_size );
			break;
		}
	}
