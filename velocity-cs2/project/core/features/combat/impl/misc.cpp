...
namespace features::combat {
	...
	void misc::autostop::on_create_move( systems::input::usercmd* cmd )
	{
		...
		const auto firing_this_tick = features::combat::g_rage.is_firing_this_tick( );

		// If the "stop in air" toggle is off but we are actually shooting this tick,
		// still apply in-air stopping to kill any residual movement on the fire tick.
		const auto in_air_ok = on_ground || config.autostop_in_air.value || firing_this_tick;
		...
		// Aggressively neutralize all user movement input first. Clear buttons
		// and impulse values so held keys cannot keep adding momentum while we stop.
		{
			auto buttons = cmd->buttons.value;
			buttons &= ~static_cast< std::uintptr_t >( cstypes::command_buttons::in_forward | cstypes::command_buttons::in_back | cstypes::command_buttons::in_moveleft | cstypes::command_buttons::in_moveright );
			cmd->buttons.value = buttons;
		...
		// Cancel any queued subtick movement presses/releases the player may have
		// generated this tick so user input cannot fight the stop impulse.
		const auto step_count = base->subtick_moves_size( );
		for ( auto i = 0; i < step_count; ++i )
		{
			...
			const auto btn = step->button( );
			if ( btn == cstypes::command_buttons::in_forward ||
				 btn == cstypes::command_buttons::in_back ||
				 btn == cstypes::command_buttons::in_moveleft ||
				 btn == cstypes::command_buttons::in_moveright )
			{
				step->set_button( 0 );
		...
		if ( speed <= 1.0f )
		{
			// Even if velocity is negligible, duck mode still asks for crouch.
			if ( config.autostop_duck.value && on_ground )
			{
				const auto is_ducked = ( prestate.flags & cstypes::entity_flags::ducking ) != 0;
				const auto ducking_state = movement_services && memory::read<bool>( movement_services + SCHEMA( "CPlayer_MovementServices", "m_bDucking"_hash ) );
				if ( !is_ducked && !ducking_state )
				{
					cmd->buttons.value |= cstypes::command_buttons::in_duck;
				}
			}
		...
		// Duck mode: hold crouch while stopping on ground to get extra friction (0.34x).
		// Avoid conflicting with duckpeek release-for-shot state.
		if ( config.autostop_duck.value && on_ground && !features::combat::g_rage.should_release_duck_for_shot( ) )
		{
			cmd->buttons.value |= cstypes::command_buttons::in_duck;
		}
		...
		if ( on_ground )
		{
			...
			const auto accel_base = this->get_effective_accel_base( local.pawn, movement_services, prestate.flags | ( config.autostop_duck.value ? cstypes::entity_flags::ducking : 0 ), ctx.weapon_max_speed );
			...
			wish_x = -velocity.x / speed;
			wish_y = -velocity.y / speed;
			...
			move_magnitude = std::clamp( speed / ctx.weapon_max_speed, 0.0f, 1.0f );
		}
		else
		{
			// In-air stopping: directly push against the current 2D velocity.
			// Source air-acceleration limits wish_speed to sv_airaccelerate * maxspeed.
			const auto sv_airaccelerate = CONVAR ("sv_airaccelerate")->get<float>( );
			const auto surface_friction = prestate.surface_friction;
			const auto max_speed = ctx.weapon_max_speed > 0.0f ? ctx.weapon_max_speed : 260.0f;
			const auto air_accel_speed = std::fminf( sv_airaccelerate * max_speed * surface_friction * cstypes::tick_interval, speed );

			wish_x = -velocity.x / speed;
			wish_y = -velocity.y / speed;

			velocity.x += wish_x * air_accel_speed;
			velocity.y += wish_y * air_accel_speed;

			// Use magnitude that is at least 1.0 to ensure we hit the wish direction
			// fully (air movement capping still applies server side).
			move_magnitude = 1.0f;
		}
		...
		forward_move = std::clamp( ( wish_x * cy + wish_y * sy ) * move_magnitude, -1.0f, 1.0f );
		left_move = std::clamp( ( wish_x * sy - wish_y * cy ) * -move_magnitude, -1.0f, 1.0f );
