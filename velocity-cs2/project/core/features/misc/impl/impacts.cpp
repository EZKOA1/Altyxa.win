	...
	void impacts::render_logs (xdraw::draw_list& draw_list, float time) {
		...
		for (auto it = this->m_logs.begin (); it != this->m_logs.end (); ) {
			...
			if (alpha > 0.01f) {
				...
				// ==================== ПРОЗРАЧНЫЙ ФОН (как в keybinds) ====================
				// Всегда рисуем размытый фон с прозрачностью
				const auto blur_alpha = 150;
				draw_list.rect_filled_blurred (x, y, total_w, h, xdraw::corner_radius {r}, xdraw::color {255, 255, 255, static_cast<std::uint8_t>(blur_alpha * alpha)});

				// Полупрозрачный фон поверх (альфа 0.6 от исходного window_bg)
				auto bg = s.window_bg;
				bg.a = static_cast<std::uint8_t>(bg.a * alpha * 0.6f);
				draw_list.rect_filled (x, y, total_w, h, scale_alpha (bg), xdraw::corner_radius {r});
				// ============================================================================
