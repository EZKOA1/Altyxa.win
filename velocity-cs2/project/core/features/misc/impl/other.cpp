...
namespace features::misc {
	...
	void other::on_player_death( std::uintptr_t event )
	{
		...
		const auto& am = settings::g_misc.m_automessage;
		if ( !am.enabled.value )
		{
			return;
		}
		...
		if ( victim_pawn )
		{
			...
			const auto is_enemy = local.is_this_other_team( victim_team );

			if ( am.team_only.value && is_enemy )
			{
				return;
			}
		...
		const auto phrases = am.get_active_phrases( );
		if ( phrases.empty( ) )
		{
			return;
		}

		std::string selected_phrase{};
		if ( am.mode.value == settings::misc::automessage::mode_type::random )
		{
			std::random_device rd{};
			std::mt19937 gen{ rd( ) };
			std::uniform_int_distribution<std::size_t> dist{ 0, phrases.size( ) - 1 };
			selected_phrase = phrases[ dist( gen ) ];
		}
		else
		{
			selected_phrase = phrases[ this->m_automessage_seq_idx % phrases.size( ) ];
			this->m_automessage_seq_idx++;
		}

		if ( selected_phrase.empty( ) )
		{
			return;
		}

		const auto cmd_base = am.team_only.value ? xs( "say_team \"" ) : xs( "say \"" );
		std::string full_cmd{ cmd_base };
		full_cmd += selected_phrase;
		full_cmd += xs( "\"" );

		memory::call<void>( PATTERN( patterns::engine_client_cmd ), addresses::globals::source2engine_to_client, 0, full_cmd.c_str( ), 0x7ffef001 );
	}
	...
	void other::do_name_changing( )
	{
		...
		{
			constexpr std::string_view tag{ "celestial" };
