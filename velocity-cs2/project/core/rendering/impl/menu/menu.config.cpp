...
namespace rendering {
	...
	namespace detail {
		...
		auto create_open{ false };
		std::string create_name_buf{};
	...
	void menu::draw_config( float group_w )
	{
		...
		if ( detail::create_open )
		{
			xui::text_input( "##cfg_create_name", detail::create_name_buf, 64, "enter config name..." );
			...
			const auto can_create = !detail::create_name_buf.empty( );
			if ( ( xui::button( "confirm", btn_w, btn_h ) || enter_pressed ) && can_create )
			{
				config::registry::save( detail::utf8_to_wide( detail::create_name_buf ) );
				detail::needs_refresh = true;
				detail::create_open = false;
				detail::create_name_buf.clear( );
			}

			xui::layout::same_line( );

			if ( xui::button( "cancel", btn_w, btn_h ) )
			{
				detail::create_open = false;
				detail::create_name_buf.clear( );
			}
		...
		else
		{
			...
			if ( xui::button( "create", btn_w, btn_h ) )
			{
				detail::create_open = true;
				detail::create_name_buf.clear( );
				detail::confirm_delete = false;
				detail::confirm_reset = false;
			}
