		...
		if (subtab == 0) {
			...
			if (xui::begin_child ("##misc_visuals", col_w)) {
				...
				if (xui::begin_popup ("##pen_popup", 220.0f)) {
					static constexpr const char* style_names [] {"square", "circle", "circle outline"};
					xui::combo ("style##pen", reinterpret_cast<int&>(pen.crosshair_style.value), style_names, 3);
					xui::checkbox ("world physics##pen", pen.physics);
					xui::slider_float ("size##pen", pen.size.value, 2.0f, 20.0f, "%.1f");
					xui::slider_float ("outline thickness##pen", pen.outline_thickness.value, 0.5f, 4.0f, "%.1f");
			...
			if (xui::begin_child ("##misc_other", col_w)) {
				...
				auto& am = m.m_automessage;
				xui::checkbox ("auto message", am.enabled);
				if (xui::begin_popup ("##automessage_popup", 280.0f)) {
					xui::checkbox ("team only##am", am.team_only);

					static constexpr const char* mode_names [] {"random", "sequential"};
					xui::combo ("mode##am", reinterpret_cast<int&>(am.mode.value), mode_names, 2);

					xui::layout::separator ();

					using automessage_t = std::remove_reference_t<decltype(am)>;
					for (std::size_t i = 0; i < automessage_t::k_max_phrases; ++i) {
						char label [32] {};
						char placeholder [64] {};
						std::snprintf (label, std::size (label), "phrase %zu##am", i + 1);
						std::snprintf (placeholder, std::size (placeholder), "phrase %zu (leave empty to skip)", i + 1);
						xui::text_input (label, am.phrases [i].value, 128, placeholder);
					}
