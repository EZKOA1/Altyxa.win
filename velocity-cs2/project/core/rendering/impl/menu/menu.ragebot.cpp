...
namespace rendering {
	...
	void menu::draw_ragebot( float group_w ) const
	{
		...
		if ( xui::begin_child( "##ragebot_extras", col_w, 275.0f, true ) )
		{
		...
		{
			xui::checkbox( "early stop", wg.autostop_early );
			xui::checkbox( "stop in air", wg.autostop_in_air );
			xui::checkbox( "duck on stop", wg.autostop_duck );
			xui::checkbox( "move between shots", wg.autostop_move_between_shots );
			xui::end_popup( );
		}
		...
		if ( xui::begin_child( "##ragebot_antiaim", col_w ) )
		{
			...
			xui::checkbox( "hideshots", aa.hide_shots );
