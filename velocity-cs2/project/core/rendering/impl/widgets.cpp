...
namespace rendering {
	...
	// ============================================================================
	// ВОДЯНОЙ ЗНАК «CELESTIAL» — прозрачный, с размытием, цвета из темы
	// ============================================================================
	void widgets::watermark (xdraw::draw_list& draw_list) {
		...
		constexpr auto logo_icon_size {14.0f};
		constexpr auto logo_icon_pad {6.0f};
		...
		// ── НОВЫЙ ЛОГОТИП «celestial» (звезда) ────────────────────────────
		static auto logo_w = 0, logo_h = 0;
		static const auto logo = xdraw::load_svg (R"(<svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M7 0L8.57 5.43L14 7L8.57 8.57L7 14L5.43 8.57L0 7L5.43 5.43L7 0Z" fill="#FFFFFF"/></svg>)", 1.0f, &logo_w, &logo_h);

		const auto inner_h = h - inner_pad * 2.0f;
		const auto logo_draw_w = static_cast<float>(logo_w);

		// ── measure text ──────────────────────────────────────────────────
		const auto [name_tw, name_th] = xdraw::measure_text ("celestial");
		...
		// ── ОСНОВНОЙ ФОН (прозрачный с размытием) ───────────────────────
		const auto blur_alpha = 160;
		draw_list.rect_filled_blurred (x, y, w, h, xdraw::corner_radius {r}, xdraw::color {255, 255, 255, static_cast<std::uint8_t>(blur_alpha)});

		auto bg = s.window_bg;
		bg.a = static_cast<std::uint8_t>(bg.a * 0.5f);
		draw_list.rect_filled (x, y, w, h, bg, xdraw::corner_radius {r});

		auto cx = x + inner_pad;

		// ── ВСПОМОГАТЕЛЬНЫЕ ЛЯМБДЫ ДЛЯ ОТРИСОВКИ ПИЛЛЕЙ ──────────────────
		...
		// ── ЛОГОТИП + НАЗВАНИЕ (цвета из темы) ────────────────────────────
		auto accent_col = s.accent;
		draw_list.rect_filled (cx, y + inner_pad, logo_pill_w, inner_h, accent_col, xdraw::corner_radius {inner_r});
		if (logo) {
			const auto iy = y + (h - static_cast<float>(logo_h)) * 0.5f;
			draw_list.image (cx + logo_icon_pad, iy, static_cast<float>(logo_w), static_cast<float>(logo_h), logo.Get (), s.checkbox_mark_icon);
		}
		draw_list.text (cx + logo_icon_pad + logo_draw_w + logo_icon_pad,
			y + (h - name_th) * 0.5f + text_nudge, "celestial", s.checkbox_mark_icon);
	...
	// ============================================================================
	// KEYBINDS — прозрачный вертикальный список с анимациями
	// ============================================================================
	void widgets::keybinds (xdraw::draw_list& draw_list) {
		...
		constexpr auto text_pad_x {8.0f};
		constexpr auto text_nudge {0.5f};
		constexpr auto icon_size {16.0f};
		constexpr auto icon_inner_pad {3.0f};
		...
		for (const auto setting : xui::binds::all ()) {
			...
			bool is_rage_group {false};
			for (auto i = 0u; i < settings::combat::ragebot::k_group_count; ++i) {
				const auto& g = settings::g_combat.m_ragebot.groups [i];
				if (setting == &g.min_damage_override || setting == &g.hitchance_override ||
					 setting == &g.force_shot || setting == &g.force_shot_air || setting == &g.body_aim ||
					 setting == &g.silent || setting == &g.no_spread) {
					is_rage_group = true;
					break;
				}
			}
			...
			bool is_legit_group {false};
			for (auto i = 0u; i < settings::combat::legitbot::k_group_count; ++i) {
				const auto& g = settings::g_combat.m_legitbot.groups [i];
				if (setting == &g.aimbot || setting == &g.rcs || setting == &g.standalone_rcs ||
					 setting == &g.triggerbot || setting == &g.autowall || setting == &g.visualize_fov ||
					 setting == &g.trigger_head_only || setting == &g.give_me_your_seed) {
					is_legit_group = true;
					break;
				}
			}
			...
			if (setting == &settings::g_combat.m_antiaim.enabled ||
				 ...
				 setting == &settings::g_combat.m_antiaim.at_target) {
		...
		for (int i = 0; i < count; ++i) {
			...
			if (e.has_value_pill)
				text += " " + std::string (e.value);
			const auto [tw, th] = xdraw::measure_text (text.c_str ());
			const auto row_w = inner_pad + tw + text_pad_x * 2.0f + inner_pad;
			if (row_w > max_row_w) max_row_w = row_w;
		}
		const auto final_block_w = std::max (header_w, max_row_w);

		draw_list.rect_filled_blurred (x, base_ry, final_block_w, total_h, xdraw::corner_radius {r}, xdraw::color {255, 255, 255, static_cast<std::uint8_t>(150 * master_alpha)});
		draw_list.rect_filled (x, base_ry, final_block_w, total_h, s.window_bg.alpha (static_cast<std::uint8_t>(s.window_bg.a * master_alpha * 0.5f)), xdraw::corner_radius {r});
		...
		for (int i = 0; i < count; ++i) {
			...
			const auto [tw, th] = xdraw::measure_text (display_name.c_str ());
			const auto row_w = inner_pad + tw + text_pad_x * 2.0f + inner_pad;
			const auto row_x = x + (final_block_w - row_w) * 0.5f;

			draw_list.rect_filled (row_x, draw_y, row_w, row_h, s.child_bg.alpha (static_cast<std::uint8_t>(s.child_bg.a * row_alpha * 0.7f)), xdraw::corner_radius {inner_r});
			const auto text_col = (e.mode == xui::bind_mode::toggle) ? s.text_dim : s.accent;
			draw_list.text (row_x + text_pad_x, draw_y + (row_h - th) * 0.5f + text_nudge, display_name.c_str (), text_col.alpha (static_cast<std::uint8_t>(text_col.a * row_alpha)));
		...
		for (auto it = row_states.begin (); it != row_states.end (); ) {
			if (!it->second.active_this_frame) {
				...
				const auto [tw, th] = xdraw::measure_text (it->first.c_str ());
				const auto row_w = inner_pad + tw + text_pad_x * 2.0f + inner_pad;
				const auto row_x = x + (final_block_w - row_w) * 0.5f;

				draw_list.rect_filled (row_x, draw_y, row_w, row_h, s.child_bg.alpha (static_cast<std::uint8_t>(s.child_bg.a * row_alpha * 0.7f)), xdraw::corner_radius {inner_r});
				draw_list.text (row_x + text_pad_x, draw_y + (row_h - th) * 0.5f + text_nudge, it->first.c_str (), s.text_dim.alpha (static_cast<std::uint8_t>(s.text_dim.a * row_alpha)));
			}
			++it;
		}
	}
